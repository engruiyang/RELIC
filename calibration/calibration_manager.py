...
class CalibrationManager:
    ...
    def start_calibration(self, user: dict, calibration_type: str, gyro_snapshots: list[dict], attention_snapshots: list[dict]) -> CalibrationProfile:
        if self.store is None:
            raise ValueError("store is required")
        history = self.list_calibrations(user["user_id"]) if user["user_type"] != "guest" else []
        has_history = bool(history)
        resolved_type = calibration_type
        if calibration_type == "auto":
            resolved_type = "quick_check" if has_history else "first_profile"

        historical_baseline = history[-1].get("attention_baseline") if history else None
        cp = self.run_quick_calibration(
            user_id=user["user_id"],
            user_type=user["user_type"],
            device_id="mock_device",
            gyro_snapshots=gyro_snapshots,
            attention_snapshots=attention_snapshots,
            has_history=(resolved_type == "quick_check"),
    ...
    def cancel_calibration(self, user: dict, reason: str = "cancelled_by_user") -> CalibrationProfile:
        return CalibrationProfile(
            ...
            calibration_type="triggered",
            attention_baseline=None,
            attention_std=None,
            attention_valid_sample_ratio=0.0,
            gyro_bias_x=None,
            gyro_bias_y=None,
            gyro_bias_z=None,
            gyro_noise_x=None,
    ...
    def list_calibrations(self, user_id: str) -> list[dict]:
        return [] if self.store is None else self.store.list_calibration_profiles(user_id)

    def get_calibration(self, calibration_id: str) -> dict | None:
        return None if self.store is None else self.store.get_calibration_profile(calibration_id)
    ...
    def bind_calibration_to_profile(self, user_id: str, calibration_id: str) -> tuple[str | None, str]:
        ...
        if cp["user_id"] != user_id:
            raise ValueError("calibration_user_mismatch")
        if not bool(cp["valid"]):
    ...
    def _compute_metrics(self, gyro_snapshots: list[dict], attention_snapshots: list[dict], historical_baseline: float | None):
        ...
        if nrms > self.gyro_noise_rms_threshold:
            return False, "gyro_unstable", self._empty_metrics(gyro_ratio, 0.0) | {"gyro_bias_x": bx, "gyro_bias_y": by, "gyro_bias_z": bz, "gyro_noise_x": nx, "gyro_noise_y": ny, "gyro_noise_z": nz, "gyro_noise_rms": nrms, "gyro_stability_score": max(0.0, 1.0 - nrms / self.gyro_noise_rms_threshold)}
        ...
        if historical_baseline is not None and abs(abaseline - historical_baseline) > self.attention_shift_threshold:
            return False, "attention_baseline_shift", self._empty_metrics(gyro_ratio, att_ratio) | {"attention_baseline": abaseline, "attention_std": astd}
