...
def evaluate(rows: list[dict], labels: list[dict], cfg: dict, label_meta: dict | None = None, use_recorded_prediction: bool = False) -> dict:
    ...
    for r in rows:
        ...
        frame_events_by_session.setdefault(sid, []).append({
            "timestamp_ms": t,
            "predicted_label": state,
            "control_state": state,
            "control_state_reason": r.get("control_state_reason", ""),
            "attention": r.get("attention"),
            "attention_age_ms": r.get("attention_age_ms"),
            "attention_fresh": r.get("attention_fresh"),
            "gyro_x": r.get("gyro_x"),
            "gyro_y": r.get("gyro_y"),
            "gyro_z": r.get("gyro_z"),
            "gyro_age_ms": r.get("gyro_age_ms"),
            "gyro_fresh": r.get("gyro_fresh"),
            "sqi": s.get("sqi"),
            "quality_state": s.get("quality_state"),
            "q_attention": s.get("q_attention"),
            "q_gyro": s.get("q_gyro"),
            "q_motion": s.get("q_motion"),
            "q_stream": s.get("q_stream"),
            "gyro_rate_rms": s.get("gyro_rate_rms"),
            "gyro_jitter_rms": s.get("gyro_jitter_rms"),
            "gyro_offset_rms": s.get("gyro_offset_rms"),
            "p_rate": s.get("p_rate"),
            "p_jitter": s.get("p_jitter"),
    ...
    if (label_meta or {}).get("mode") == "frames":
        ...
        for fr in labels:
            ...
            events = [ev for ev in frame_events_by_session.get(sid, []) if a <= int(ev["timestamp_ms"]) < b]
            in_frame = [ev["predicted_label"] for ev in events]
            if not in_frame:
                continue
            counts = {}
            for s in in_frame:
            ...
            if best != label:
                representative = events[-1] if events else {}
                misclassified_rows.append({
                    ...
                    **{k: representative.get(k, "") for k in [
                        "attention", "attention_age_ms", "attention_fresh", "gyro_x", "gyro_y", "gyro_z", "gyro_age_ms",
                        "gyro_fresh", "sqi", "quality_state", "control_state", "control_state_reason", "q_attention",
                        "q_gyro", "q_motion", "q_stream", "gyro_rate_rms", "gyro_jitter_rms", "gyro_offset_rms", "p_rate",
                        "p_jitter", "p_offset", "warning_flags", "error_flags", "fi_raw", "fi_smoothed", "fi_valid"
                    ]},
                })
        transition_count = 0
    else:
...
def main():
    ...
    p.add_argument("--debug-misclassified-out")
    p.add_argument("--use-recorded-prediction", action="store_true")
    a = p.parse_args()
    ...
    out = evaluate(rows, labels, cfg, label_meta=label_meta, use_recorded_prediction=a.use_recorded_prediction)
    ...
    if a.debug_misclassified_out:
        ...
        with open(a.debug_misclassified_out, "w", encoding="utf-8", newline="") as f:
            w = csv.DictWriter(f, fieldnames=cols)
            w.writeheader()
            for row in out.get("misclassified_frames", []):
                rec = dict(row)
                rec["warning_flags"] = json.dumps(rec.get("warning_flags", ""))
                rec["error_flags"] = json.dumps(rec.get("error_flags", ""))
