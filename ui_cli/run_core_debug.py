...
from relic_core.models import InputEvent
...
def main():
    ...
    try:
        for i in range(1,a.ticks+1):
            ...
            print(f"tick={i} connected={s.get('connected')} attention={s.get('attention_value')} age={s.get('attention_age_ms')} focus=({s.get('focus_x')},{s.get('focus_y')}) gyro=({s.get('gyro_x')},{s.get('gyro_y')},{s.get('gyro_z')}) sqi={q.get('sqi'):.2f} fi={f.get('fi'):.1f} state={out['state']} game={out['current_game']}")
