...
def test_trace_lock_outputs_replaceable_asset_and_effect_keys_without_paths() -> None:
    c = TraceLockClient()
    c.start({"session_id": "s1", "difficulty": 5})
    view = c.build_game_view()
    target = [e for e in view.entities if e.kind == "target"][0]
    ring = [e for e in view.entities if e.kind == "progress_ring"][0]
    timer = [e for e in view.entities if e.kind == "timer_bar"][0]

    for key in [target.asset_key, target.style_key, ring.asset_key, ring.style_key, timer.asset_key, timer.style_key]:
        assert key.startswith("tracelock.")
        assert "/" not in key
        assert not key.endswith((".png", ".webp", ".svg"))

    c.handle_input(_evt(x=target.x, y=target.y, ts=100))
    fx_view = c.build_game_view()
    assert fx_view.visual_events
    for fx in fx_view.visual_events:
        assert fx.effect_key.startswith("tracelock.effect.")
        assert fx.style_key == fx.effect_key
        assert "/" not in fx.effect_key
        assert not fx.effect_key.endswith((".png", ".webp", ".svg"))
