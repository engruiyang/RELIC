...
def _get_nested_algorithm_data(message: dict[str, Any]) -> tuple[str | None, Any]:
    algorithm_name = message.get("algorithm_name") or message.get("algorithm_name:")
    result_args = _ensure_dict(message.get("result_args"))
    data = result_args.get("data")
    if isinstance(data, str):
        text = data.strip()
        if text.startswith("{") and text.endswith("}"):
            decoded = _ensure_dict(text)
            if decoded:
                data = decoded
    return algorithm_name, data
            ...
            if algorithm_name == "attention":
                ...
                if data_dict:
                    ...
                    if gyro_sample:
                        ...
                        if self.print_attention:
                            print(
                                f"陀螺仪: focus=({(self.focus_x if self.focus_x is not None else 0):.2f}, {(self.focus_y if self.focus_y is not None else 0):.2f}), "
                                f"gyro=({(self.gyro_x if self.gyro_x is not None else 0):.2f}, {(self.gyro_y if self.gyro_y is not None else 0):.2f}, {(self.gyro_z if self.gyro_z is not None else 0):.2f})"
                            )
            else:
                if self.print_attention:
                    print(f"算法输出: algorithm_name={algorithm_name}, data={data}")
