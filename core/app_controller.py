...
class AppController:
    def __init__(self):
        ...
        self.state_machine=StateMachine(); self.device_manager=DeviceManager(); self.data_center=DataCenter(self.config.get('data_center',{})); self.tick_count=0
    def start(self)->None:
    ...
    def tick(self,dt_ms:int=50)->dict:
        now=self.clock.monotonic_ms(); events=self.device_manager.poll_events(dt_ms); self.data_center.ingest_events(events,now); snap=self.data_center.tick(now); self.tick_count+=1
        return snap.to_dict()
