...
import "../components"
Item { property var controlStateObj: ({}); property string commandSummary:""; property string selectedCommandId:""; property string selectedStatus:""; property string selectedExecutionMode:""; property string selectedNativeActionId:""; property string selectedCliReference:""
 function pick(id,status,mode,cli){ selectedCommandId=id; selectedStatus=status; selectedExecutionMode=mode; selectedNativeActionId=""; selectedCliReference=cli }
 function s(v){return (v===undefined||v===null||v==="")?"n/a":String(v)}
 Column { anchors.fill: parent; spacing:6
  PageHeader { titleText:"Developer Lab"; subtitleText:"Advanced/manual/copy_only command catalog" }
  GroupBox { title:"Developer Lab Actions"; Column {
  ...
  GroupBox { title:"Metadata"; Column { Label{text:"execution_mode: "+s(selectedExecutionMode)}; Label{text:"danger_level: advanced"}; Label{text:"writes_db: possible"}; Label{text:"connects_platform: possible"}; Label{text:"generates_files: likely"}; Label{text:"cli_reference: "+s(selectedCliReference)}; Label{text:"copy_only / manual"} } }
  GroupBox { title:"Page Commands"; Label{text: commandSummary; wrapMode: Text.WordWrap} }
  PageFeedbackPanel { pageId:"developer_lab"; selectedCommandId: parent.selectedCommandId; selectedStatus: parent.selectedStatus; selectedExecutionMode: parent.selectedExecutionMode; selectedNativeActionId: parent.selectedNativeActionId; lastCommand:s(controlStateObj.last_command); lastResult:s(controlStateObj.last_command_result); lastError:s(controlStateObj.last_command_error) }
 }
}
