...
Item {
    ...
    function task26TrainingLayoutPayload() {
        var resources = trainingPage.renderResourcesObj || ({})
        return resources.task26_training_layout_payload || ({})
    }

    function task26TrainingLayoutStatus() {
        var resources = trainingPage.renderResourcesObj || ({})
        return (resources.task26_training_layout_status === undefined || resources.task26_training_layout_status === null)
            ? "n/a"
            : String(resources.task26_training_layout_status)
    }

    function task26TrainingLayoutSource() {
        var resources = trainingPage.renderResourcesObj || ({})
        return (resources.task26_training_layout_source === undefined || resources.task26_training_layout_source === null)
            ? "n/a"
            : String(resources.task26_training_layout_source)
    }
    ...
    ScrollView {
        ...
        Column {
            ...
            GroupBox {
                title: "TASK26 Training Desktop Pilot" // required card: game_canvas_card
                width: parent.width
            ...
            GroupBox {
                ...
                visible: trainingPage.task26LegacyFallbackVisible && (activePanel === "session" || activePanel === "readiness" || activePanel === "result")
                width: parent.width
                Column {
                    width: parent.width
                    spacing: 4
            ...
            GroupBox {
                ...
                Column {
                    ...
                    Label { text: "TraceLock / Fragment Lock / 纰庣墖閿佸畾: existing game client active" }
                    Label { text: "Signal Hunter / 淇″彿鐚庢墜: planned" }
                    Label { text: "Stabilizer / 绋冲畾鍗忚: planned" }
                }
            }
