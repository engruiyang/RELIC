...
class DataCenter:
    ...
    def _update_control_state(self, s: RealtimeSnapshot, now_ms: int) -> None:
        if not s.stream_alive:
            ...
            return
        if (s.attention is None) or (not s.attention_seen_once):
            self._low_fi_since = None
        ...
        if s.recovering:
            ...
            return
        if s.quality != 'ok' or (not s.fi_valid) or (not s.control_data_valid):
            self._low_fi_since = None
    ...
    def tick(self, now_ms: int) -> RealtimeSnapshot:
        ...
        has_attention = s.attention is not None and s.attention_seen_once
        s.training_data_valid = bool(
            ...
            motion_fresh and has_attention and not s.error_flags and s.sqi >= self.cfg['min_sqi']
        )
        if s.training_data_valid and s.quality == 'ok' and not s.recovering:
            self._quality_ok_streak += 1
        else:
            self._quality_ok_streak = 0
        s.control_data_valid = bool(s.training_data_valid and s.quality == 'ok' and self._quality_ok_streak >= self.cfg['recovering_ok_ticks'])
        ...
        if s.quality == 'warning' and not self.cfg.get('warning_allows_fi_valid', False):
            s.fi_valid = False
        if (s.attention is None) or (not s.attention_seen_once):
            s.training_data_valid = False
            s.control_data_valid = False
            s.fi_valid = False
