...
class DataCenter:
    ...
    def _update_control_state(self,s:RealtimeSnapshot,now_ms:int)->None:
        ...
        if low_dur>=self.cfg['fatigued_enter_ms'] and s.training_data_valid and s.quality=='ok':
            s.control_state='FATIGUED'; self._recover_streak=0; return
        if s.control_state=='FATIGUED':
            if fi>=45: self._recover_streak+=1
            else: self._recover_streak=0
            if self._recover_streak*100>=self.cfg['fatigued_exit_ms']: s.control_state='DISTRACTED'
            return
    ...
    def get_snapshot(self)->RealtimeSnapshot: return self._s
